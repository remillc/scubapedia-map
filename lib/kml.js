...
function getCoordinates(htmlPage, name) {
  ...
  try {
    ...
    let data = $page.find('#map_google3_1 > .mapdata').text();
    // console.log(htmlPage)
    if (data) {
      data = JSON.parse(data);
      if ('locations' in data && data.locations.length === 1) {
    ...
    if ($urls.length > 0) {
      const url = $urls.first().attr('href')
      ...
      if (typeof url !== 'undefined') {
        ...
        const coords = gmapUrl.searchParams.get('ll').split(',');
