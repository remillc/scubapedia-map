      ...
      axios
        ...
        .then(response => {
          ...
          try {
            ...
            this.url = url;
            this.description = getDescription(html, url, this.title)
...
function getCoordinates(htmlPage, name) {
  ...
  try {
    ...
    // const data = JSON.parse($page.find('#map_google3_1 .mapdata').html())
    // console.log(data)
    // process.exit()
...
function getDescription(html, url, title) {
  const $description = $(`<div><base href="${url}" />${$(html).find('#bodyContent').html()}</div>`)
  $description.find('#siteSub, #jump-to-nav, .printfooter, .mw-jump-link, #contentSub, #map_google3_1').remove()
  $description.prepend(`<a href="${url}">Lien Scubapedia</a>`)
...
async function main() {
  try {
    ...
    const kml = template(sites.filter(site => site.coordinates))

    const kmz = archiver('zip', {
      zlib: { level: 9 } // Sets the compression level.
    });

    const kmzFile = fs.createWriteStream(join(__dirname, '..', 'scubapedia.kmz'))
